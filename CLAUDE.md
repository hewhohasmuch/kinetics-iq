...
**Face redaction.** The patient's head is blurred out of the live preview and both snapshots, using one code path for both: `headRegion()` (`src/core/headRegion.js`, pure geometry) locates a head circle from the face and shoulder landmarks; `PoseDetector.detect()` returns it as `head`; `Overlay.draw()` blurs it into the overlay canvas **before** the landmark dots and before its own no-markers early return; and `_captureFrameTo()` composites that *same* overlay canvas into the retained snapshot. The stored JPEG therefore inherits whatever the preview drew — do not add a second blur at capture or encode time, or the two can disagree.
...
Three constraints in `headRegion.js` are load-bearing, not tuning knobs: the region is computed from landmark **position with no visibility threshold** (a confidence gate no-ops in exactly the hard cases — backlit, prone face-down — where the guarantee matters most); the redaction is drawn **fully opaque** (`overlay.js`'s helpers leave `globalAlpha` dirty, and any transparency lets the sharp face through); and where Canvas 2D `filter` is unsupported it degrades to an **opaque solid fill**, never to nothing.

**Containment is an invariant, not a hope.** The radius used to come only from heuristics (`max(sFace, sTorso) × HEAD_RADIUS_FACTOR`) and merely *tended* to cover the face; a close-framed profile pose (the side-on knee/hip shot, where `sTorso` takes over while `CRANIUM_NUDGE` pushes the centre up toward the cranium) left the mouth landmarks ~5% of r **outside** the circle. `headRegion()` now expands the radius after placing the centre: `r = max(r_heuristic, maxDistanceToAnyFaceLandmark × COVERAGE_MARGIN)`, with `COVERAGE_MARGIN = 1.60`.

**The margin carries the whole skull, which is why it is that large.** MediaPipe's eleven face landmarks bound the *face* — eye line to mouth, ear to ear. The cranium, hairline and the back of the head extend well past them and have **no landmarks of their own**, so containment can never reach them by construction; only the margin can. It was tuned by looking at the exported e2e snapshot, not derived: at 1.10 the face was covered but hair, ear and the back of the head were left **sharp in the stored image**. Re-tune it the same way — `E2E_DIAG=1 npm run verify:e2e`, then open `.fixtures/peak-analysed.png` and look.

One consequence to know before touching this: at 1.60 containment out-reaches the heuristic in every realistic pose, so **`sFace` and `HEAD_RADIUS_FACTOR` no longer decide the radius** — deleting `sFace` leaves the whole suite green. The heuristic's remaining jobs are placing the centre (via `CRANIUM_NUDGE × r_heuristic`) and acting as a floor when the face landmarks give no spread at all (occlusion, or the subject far from the camera), which is what `keeps the heuristic as a floor when the face landmarks give no spread` pins. Don't read the surviving `max(sFace, sTorso)` as load-bearing for size; it isn't, at this margin.

**`MAX_RADIUS_FRACTION` is applied last and wins**: on a garbage landmark frame the sanity cap overrides containment, so containment holds only while the radius is uncapped. The cap also bounds the heuristic radius that places the centre — without that, a nonsense scale nudges the centre off-screen and the region is discarded as off-frame, i.e. no redaction at all on the frames least worth trusting.

**The blur is applied on the scratch canvas, not the overlay canvas.** `resize()` leaves the overlay context with a `dpr` CTM, and whether Canvas 2D `filter` blur lengths scale with the CTM is unresolved across engines — if they don't, a blur set there lands at 1/dpr strength (≈0.117 × head radius on a dpr-3 iPhone) while the session still stamps `'blur1'`. The scratch context has the identity CTM, so the question is moot: the scratch is sized `ceil(g.size × dpr)`, filled opaque *first* and unfiltered, blurred at `g.blurRadius × dpr`, then blitted to the overlay **unfiltered** inside the circular clip. Don't move the blur back, and don't add a second one at the overlay. `redactionGeometry()`'s `padding = 2 × blurRadius` coupling stays intact because both terms scale by the same `dpr`. Feature detection probes the **scratch** context, since that is where the filter is applied.

`faceRedaction` on the session records which mode the device actually used (`'blur1'`, or `'solid1'` for the no-filter fallback). It asserts the **pipeline was active**, nothing more — it does *not* assert a face was blurred in every frame, since a session can run entirely with the head off-frame (ankle work) and still stamp `'blur1'`. Sessions without the field predate this feature and were captured with the face unredacted.

**The video frame is read once per tick, into a buffer, and everyone shares it.** `MeasureView._runDetection()` used to read the live `<video>` element three separate times — once for `detector.detect()`, once inside `overlay.draw()` for the blur source, once in `_captureFrameTo()` for the stored photo. A live video element can return a different decoded frame on each read; three reads landing on three different frames would position the blur circle from one frame's landmarks while sourcing pixels from another, offsetting the redaction from the face in the stored JPEG. `_runDetection()` now blits the video into a persistent `this._frameCanvas` once, at the top of the tick, and passes that canvas — not `videoEl` — to `detect()`, to `overlay.draw()`'s `video` option, and to `_captureFrameTo()`. All three see byte-identical pixels by construction. Do not reintroduce a direct `videoEl` read anywhere in the capture path; if a future consumer needs the current frame, hand it `this._frameCanvas`.
...
`headRegion.test.js` pins **containment** — that every one of the eleven face landmarks lies inside the returned circle — for frontal, profile, close-framed-profile and prone fixtures. That invariant is the module's whole point, and it is the one thing the heuristics alone never guaranteed. Each assertion in that file exists to reject a specific wrong implementation, and several earlier versions passed against code with the feature deleted; before changing a bound, check what mutation it catches (force `scale = sTorso`, force `scale = sFace`, drop the containment term, drop the heuristic floor) and confirm the replacement still fails.

The e2e's redaction check compares the head cells against **sharp body cells**, not against the whole-frame median. The median formulation is the obvious one and it does not work here: roughly half the fixture is sky and ocean, dragging the median to ~0.8, while a genuinely blurred face measures 1.3–3.8 — so the only way to satisfy a median-relative ceiling was for the blur to land *on the sky*, which is precisely the bug the check exists to catch. Don't "simplify" it back. The head-cell coordinates are fixture-specific; re-measure them with `E2E_DIAG=1` and by opening `.fixtures/peak-analysed.png` if the fixture or source photo changes.

Note the untested surface: `src/ui/` has no unit tests, so the wiring in `MeasureView._runDetection()` — the order of overlay draw vs snapshot capture, which value reaches the recorder — is only covered end-to-end. Verify changes there by running the app, not by reasoning about the diff. Two properties of the redaction have **no automated coverage at all** and need a device: that `_drawRedaction` actually consults `_filterSupported` (a mutant that always takes the blur branch passes every test and emits a sharp face on an engine without Canvas 2D filters), and the frame-rate cost of the per-tick frame blit.
