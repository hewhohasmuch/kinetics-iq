...
**Known limitation — the seed raise pushed the occluder onto the shoulder.** The 1.60/1.75 seed above exists to contain wind-blown hair, which has no landmarks — but the ellipse is symmetric about its centre, so growing it upward to cover hair also grows it downward onto the shoulder. Measured on an anatomically-proportioned standing shoulder-ROM framing (head ≈135px tall, face centroid 120px above the shoulder line, shoulder half-width 120px, 720×1280 frame): shoulder midpoint/neck `q=0.91` (inside the opaque core, alpha≈1.0), shoulder landmarks `q=1.18` (in the feather, alpha≈0.4). At the pre-raise seed (0.92/1.14) the same points were comfortably outside the feather (`q≈1.7–2.1`). Raising `CRANIUM_NUDGE` to move the centre away from the shoulder does not fix this: the same mouth/chin landmarks that anchor containment sit on the shoulder side of the centre, so moving the centre away from the shoulder moves it away from them too — which raises the containment factor `t` and re-inflates the ellipse back toward the shoulder about as fast as the nudge moves the centre away. A swept `(CRANIUM_NUDGE, ELLIPSE_ALONG)` parameter search (2026-08-08) found no combination that clears `FEATHER_EXTENT = 1.35` at the shoulder midpoint without either collapsing `ELLIPSE_ALONG` below `ELLIPSE_ACROSS` (a head wider than tall — see the "is longer along the head axis than across it" test) or reopening the hair-coverage regression the seed raise fixed; the best sensible point found (`CRANIUM_NUDGE=0.40`, `ELLIPSE_ALONG=1.60`, the floor equal to `ELLIPSE_ACROSS`) only reached `q=1.03`, still inside the feather. Measurement itself is unaffected — detection runs on the buffered frame before the overlay is drawn — but the stored shoulder snapshot's evidence near the acromion is reduced. This contradicts the "over-covering is free" premise in `headRegion.js`'s file-level comment for the shoulder joint specifically; that comment now says so. A real fix needs an asymmetric shape (more reach toward the cranium than toward the joint) or a per-joint seed, neither attempted here.

**`MAX_RADIUS_FRACTION = 0.50` is still applied last, to both axes, and still wins — but not because a capped frame means bad landmarks.** Measured at the current `ELLIPSE_ACROSS`/`ELLIPSE_ALONG` (1.60/1.75): the cap now engages around a 170px face width in a 720px-tall frame (it was ~200px before the seed raise) — an ordinary elbow or shoulder framing, not a garbage one. Containment itself stays comfortably satisfied there (worst landmark `q` measured 0.66–0.83) and doesn't actually lapse until roughly a 370px face width — a head filling more than half the frame, unreachable in any realistic ROM shot. `MAX_RADIUS_FRACTION` was calibrated when the seeds were ~1.0× `rHeuristic` and hasn't been revisited since they rose to 1.6–1.75×. The cap still overrides containment whenever it does engage — an ellipse must not be able to mask half the frame, whatever the landmarks say — so containment holds only while both semi-axes are uncapped. The cap also bounds the heuristic radius that places the centre — without that, a nonsense scale nudges the centre off-screen and the region is discarded as off-frame, i.e. no redaction at all on the frames least worth trusting.
