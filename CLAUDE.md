...
**The `⚠` line only knows what the stamps know.** `sessionProvenance()` reads `angleConvention`/`angleFilter` and nothing else, so a *current* shoulder session exports clean — no warning — even though the shoulder neutral is documented above as systematically off (raw sits ~25° high at rest, and Set Zero over-corrects the top of the range). The note therefore can put a number in a permanent record that this file already knows is wrong. That is not a bug in `report.js` — it is the shoulder limitation reaching a new surface, and it raises the stakes on fixing it. When it is fixed, the `angleConvention` bump makes today's shoulder sessions legacy and the warning starts firing on them retroactively, which is the correct outcome.
...
**`sessionReportModel(session)` is the shared source of every rendering.** There are two now — the clipboard note and the PDF — so `sessionNoteText()` is a plain-text join over the model and composes nothing itself. A second renderer formatting its own clinical strings is exactly the drift this prevents; `report.test.js` pins that every field the model produces reaches the note. The `warning` field is *data* (`{level, label, tail}`), carrying no glyph, because the two media mark it differently — see the encoding trap below.
...
**No patient identifier, in the document *or the filename*.** Same reasoning as the note, and it matters more: a file is more detachable than a paste — it sits in Files, it can be mailed. The filename carries joint, side and date (`kineticsiq-right-knee-2026-08-12.pdf`), enough to tell two exports apart without naming anyone. Pinned by `pdf.test.js` on both the drawn text and the filename.

The `⚠`-line caveat above applies here identically: a *current* shoulder session exports with no warning even though this file documents its neutral as systematically off, because `sessionProvenance()` only knows its stamps. The PDF just raises the stakes — it is a document filed in a chart rather than text in a note.
